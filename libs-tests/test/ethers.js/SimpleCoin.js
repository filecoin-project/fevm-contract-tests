...
describe('ethers.js SimpleCoin', function () {
  ...
  it("Should access transaction details after it has been mined", async function () {
    const txByHash = await ethers.provider.getTransaction(deploymentTxHash);

    const { blockHash, blockNumber } = txByHash;
  ...
  it("Should access transaction receipt after it has been mined", async function () {
    const txReceipt = await ethers.provider.getTransactionReceipt(
      deploymentTxHash
    );
  ...
  it("Should find the transaction in block tx list", async function () {
    ...
    [blockByHash, blockByNumber].forEach((b) => {
      rpcTests.testGetBlock(b, deploymentTxHash);
    });
  ...
  it("Should interact with the contract using eth_call", async function () {
    const SimpleCoin = await ethers.getContractAt(
      "SimpleCoin",
      simpleCoinAddress
    );
  ...
  it("Should get the contract byte code at the deployed address", async function () {
    ...
    const code = await ethers.provider.getCode(SimpleCoin.address, "pending");
  ...
  xit("Should get storage using eth_getStorageAt", async function () {
    const SimpleCoin = await ethers.getContractAt(
      "SimpleCoin",
      simpleCoinAddress
    );
